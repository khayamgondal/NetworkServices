...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def _destroy(self, instance):
        try:
            ...
            import MySQLdb
            db = MySQLdb.connect(host="172.16.10.223", # your host, usually localhost
                     user="nics", # your username
                      passwd="nics", # your password
                      db="nics") # name of the data base
            cur = db.cursor()
            insname = instance['name']
            newstate = "free"
            cur.execute ("UPDATE nics.nicsdata SET state= " + "\'" + newstate +"\'" +" WHERE state= " + "\'" + insname +"\'" )
            db.commit()
            db.close()
            LOG.info('NIC has been detached from '+ insname)
        except exception.InstanceNotFound:
            virt_dom = None
    ...
    def _get_guest_config(self, instance, network_info, image_meta,
        ...
        db = MySQLdb.connect(host="172.16.10.223", # your host, usually localhost
                     user="nics", # your username
                      ...
                      db="nics") # name of the data base
        cur = db.cursor()
        cur.execute('SELECT * from nics.nicsdata')
        for row in cur.fetchall():
            ...
            if (row[1] == 'free' ):
                ...
                insname = instance['name']
                cur.execute ("UPDATE nics.nicsdata SET state= " + "\'" + insname +"\'" +" WHERE nicname= " + "\'" + row[0] +"\'" )

                db.commit()
                
                break
            else:
                LOG.info('NO NIC is avaiable to attach ' + instance['name'])
        db.close()
